    ...
    private class timeIterator implements Iterator<Time> {
        ...
        public timeIterator() {
            for (Zombie z : zombies) {
                double nearestCivilianDistance = Double.MAX_VALUE;
                for (Civilian c : civilians) {
                    double dx = z.getX() - c.getX();
                    double dy = z.getY() - c.getY();
                    double distance = Math.sqrt(dx * dx + dy * dy);
                    if (distance < nearestCivilianDistance) {
                        nearestCivilianDistance = distance;
                        z.setDistanceToTarget(distance);
                        z.setTargetDirection(new double[]{ dx / distance, dy / distance });
                    }
                }
            }
        }
